...
# 这个脚本用于负样本挖掘，主要是利用faiss-gpu进行索引和搜索，将相似度排序后的负样本挑选出来，并输出到文件中。
# 文件格式为jsonl，每一行包括query, pos, neg三个字段，分别表示查询语句、正样本列表、负样本列表。
# 其中pos是和query最相似的句子，neg是和query最不相似的句子。
# 语料库由每一行的pos和neg组成
...
class DataArgs:
    ...
    range_for_sampling: str = field(
        default="10-210", metadata={"help": "The range to sample negatives."}   # 负样本在相似度排序后所取得的范围
    )
...
def create_index(embeddings: np.ndarray, use_gpu: bool = False):
    ...
    embeddings = np.asarray(embeddings, dtype=np.float32)   # 将numpy数组转化为float32类型
    if use_gpu:
        # 判断是否有多个gpu
        ...
        elif faiss.get_num_gpus() == 1: # 只有一个gpu
...
def batch_search(
    ...
    for start_index in tqdm(range(0, len(query), batch_size), desc="Batches", disable=len(query) < 256):
        ...
        batch_scores, batch_inxs = index.search(np.asarray(batch_query, dtype=np.float32), k=topk)      # 形状为min(batch_size, len(query)) * topk, 求得每个query的topk个结果，按照相似度从大到小顺序排序，以及对应的索引，padding部分的scores为-inf，idx为-1
        input((batch_scores.shape, batch_inxs.shape))
...
def find_knn_neg(
    ...
    q_vecs = model.encode_queries(queries)      # 10 * 768
    ...
    for i, data in enumerate(train_data):
        query = data['query']
        inxs = all_inxs[i][sample_range[0]:sample_range[1]]     # 负采样的样本
        filtered_inx = []
        for inx in inxs:
            if inx == -1: break     # padding部分的inx为-1，直接退出
            if corpus[inx] not in data['pos'] and corpus[inx] != query:     # 过滤掉正样本和query本身
                filtered_inx.append(inx)

        if len(filtered_inx) > negative_number:
            filtered_inx = random.sample(filtered_inx, negative_number)     # 随机采样负样本
        data['neg'] = [corpus[inx] for inx in filtered_inx]

    with open(output_file, 'w') as f:
        for data in train_data:
            if len(data['neg']) < negative_number:      # 如果负样本数量不足，随机从语料库中采样
